...
class Parser:
    ...
    def jump(self):
        ...
        return ins.strip()
    def value(self):
        m = re.search("@(?P<value>.*)", self.instruction)
...
def first_pass(code_file):
    ...
    for k, v in outs.items():
        ...
        while code_file[counter] != v:
            # print('in while')
            label_table[code_file[counter][1:-1]] = k
...
def main():
    ...
    # filename = sys.argv[1]
    # filename = ""
    filename = r'D:\nand2tetris\projects\06\assembler\Pong.asm'
    print(filename)
    ...
    for instruction in code_file:
        print(instruction)
        out = assemble(instruction, code)
        if out:
            outs.append(out)
    print(code.pre_define_table)
